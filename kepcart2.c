...
void cartesian(double gm, 
	       double a, double e, double i, double longnode, double argperi, double meananom, 
	       // State *state)
         double *xi, double *yi, double *zi, double *xdi, double *ydi, double *zdi)
{
  ...
  if(n>=MAX_ITER){ /* Steffensen's method failed.  Try direct iteration */
    ...
    if(n>=MAX_ITER){
      fprintf(stderr, "direct Kepler equation iteration maximum reached.\n");
      fprintf(stderr, "e: %.16lf meananom: %.16lf\n", e, meananom);
      exit(-1);
    }
  }
  ...
  // state->zd = zd;
  *xi = x * cosnode - y * sinnode;
  *yi = x * sinnode + y * cosnode;
  *zi = z;
  *xdi = xd * cosnode - yd * sinnode;
  *ydi = xd * sinnode + yd * cosnode;
  *zdi = zd;
